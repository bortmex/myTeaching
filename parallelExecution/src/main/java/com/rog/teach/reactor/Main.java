...
public class Main {
    public static void main(String[] args) throws InterruptedException {
        ...
        Flux<Object> telegramProduces = Flux
                .generate(
                        () -> 2354,
                        (state, sink) -> {
                            if (state > 2366) {
        ...
        Flux
                .create(sink -> {
                    telegramProduces.subscribe(new BaseSubscriber<Object>() {
                        @Override
                    ...
                    sink.onRequest(r -> {
                                sink.next("DB returns: " + telegramProduces.blockFirst());
                            }
                    );
                })
                .subscribe(System.out::println);
